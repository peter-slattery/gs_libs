...
typedef void* gs_memory_realloc(void* Address, gs_mem_u32 Size);
...
static gs_mem_u8*
ArenaRealloc(memory_arena* Arena, gs_mem_u8* Head, gs_mem_u32 Size)
{
    gs_mem_u8* Result = 0;
    
    if (Arena->Realloc != 0)
    {
        Result = (gs_mem_u8*)Arena->Realloc(Head, Size);
    }
    else
    {
#ifdef GS_MEMORY_NO_STD_LIBS
        ...
        GSMem_Assert(0);
        #else
         Result = (gs_mem_u8*)realloc(Head, Size);
        #endif
    }
...
GrowArena(memory_arena* Arena, gs_mem_u32 SizeNeeded)
{
    ...
    GSMem_Assert(!(Arena->ExpansionRule == MemoryExpansion_OnlyIfFunctionsProvided 
             && (Arena->Alloc == 0 || Arena->Realloc == 0)));
    
gs_mem_u32 NewBuffersCount = (Arena->BuffersCount + 1);
    gs_mem_u32 NewBuffersSize = sizeof(memory_buffer) * NewBuffersCount;
    Arena->Buffers = (memory_buffer*)ArenaRealloc(Arena, (gs_mem_u8*)Arena->Buffers, NewBuffersSize);
...
address_and_buffer ResultAddress = FindAlignedAddressInBufferWithRoom(Arena, AllocationSize, Alignment, AlignmentMask);
