...
#define GS_MEMORY_BUFFER_SIZE Megabytes(2)
#endif
...
#define AllocStruct(arena, type) (type*)Alloc_(arena, sizeof(type))
#define AllocArray(arena, type, count) (type*)Alloc_(arena, sizeof(type) * count)
#define AllocSize(arena, size) Alloc_(arena, size)
static u8*
Alloc_(memory_arena* Arena, u32 Size)
{
    GSMem_Assert(Arena->Alloc != 0);
    u8* Result = (u8*)Arena->Alloc(Size);
    return Result;
}

#define FreeStruct(arena, base, type) Free_(arena, (u8*)base, sizeof(type))
#define FreeArray(arena, base, type, count) Free_(arena, (u8*)base, sizeof(type) * count)
#define FreeSize(arena, base, size) Free_(arena, (u8*)base, size)
static void
Free_(memory_arena* Arena, u8* Base, u32 Size)
{
    GSMem_Assert(Arena->Free != 0);
    Arena->Free(Base, Size);
}

#define ReallocArray(arena, base, type, oldcount, newcount) (type*)Realloc_(arena, (u8*)base, oldcount * sizeof(type), newcount * sizeof(type))
#define ReallocSize(arena, base, oldsize, newsize) Realloc_(arena, (u8*)base, oldsize, newsize)
static u8*
Realloc_(memory_arena* Arena, u8* Base, u32 OldSize, u32 NewSize)
{
    u8* Result = 0;
    if (Arena->Realloc != 0)
    {
        Result = (u8*)Arena->Realloc(Base, OldSize, NewSize);
    }
    else if (Arena->Alloc != 0 && Arena->Free != 0)
    {
        Result = (u8*)Arena->Alloc(NewSize);
        GSMemCopy(Base, Result, OldSize);
        Arena->Free(Base, OldSize);
    }
    else
    {
        InvalidCodePath;
...
ArenaRealloc(memory_arena* Arena, gs_mem_u8* Head, gs_mem_u32 OldSize, gs_mem_u32 NewSize)
{
    ...
    if (Arena->Alloc != 0 && Arena->Free != 0)
    {
        Result = (gs_mem_u8*)Arena->Alloc(NewSize);
        for (u32 i = 0; i < OldSize; i++)
        {
            Result[i] = Head[i];
        }
        Arena->Free(Head, OldSize);
    }
    else if (Arena->Realloc != 0)
    {
