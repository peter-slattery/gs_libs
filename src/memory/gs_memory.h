...
void
CopyMemoryTo(u8* From, u8* Dest, u64 Size)
{
...
Alloc_(gs_allocator A, u64 Size, gs_debug_loc Loc, char* ArenaName)
{
  ...
  // Debug
  {
    ...
    SLLPushOrInit(Allocation, A.DEBUGAllocList->Root, A.DEBUGAllocList->Head);
    ...
    {
      A.DEBUGAllocList->ArenaHashes[ArenaIndex] = ArenaHash;
      A.DEBUGAllocList->ArenaInfos[ArenaIndex] = (gs_debug_arena_info){
        .ArenaName = ArenaName,
      };
    }
...
#define DEBUG_ALLOCATOR_ARENA_MAX 256
  ...
  *Result.DEBUGAllocList = (gs_debug_allocations_list){
    .ArenaHashesCount = DEBUG_ALLOCATOR_ARENA_MAX,
    .ArenaHashes = (u64*)PAlloc(sizeof(u64*) * DEBUG_ALLOCATOR_ARENA_MAX, 0, UserData),
    .ArenaInfos = (gs_debug_arena_info*)PAlloc(sizeof(gs_debug_arena_info) * DEBUG_ALLOCATOR_ARENA_MAX, 0, UserData),
  };
...
MemoryCursorPushSize_(gs_memory_cursor* C, u64 Size)
{
  ...
  {
    Result = (gs_data){
      .Memory = C->Data.Memory + C->Position,
      .Size = Size,
    };
    C->Position += Size;
  }
